...
from modeling import BertConfig, CrossEntropyWrapper, NextSentencePrediction
...
parser.add_argument('--checkpoint_dir', default='/home/user/yzy/vpipe-main-1/runtime/bert/', type=str, metavar='PATH',
...
def main():
    ...
    # criterion = NextSentencePrediction()
    ...
    for module_id, (stage, inputs, outputs) in enumerate(model[:-1]):
        input_tensors = []
        ...
        def count_parameters_in_MB(model):
            ...
            param_size = num_params * 4  # 每个参数默认为32-bit浮点数, 即4个字节
            param_size_in_MB = param_size / (1024 * 1024)  # 将字节转换成兆字节（MB）
            return param_size_in_MB
        with torch.no_grad():
            output_tensors = stage(*tuple(input_tensors))
        print(count_parameters_in_MB(stage))
    ...
    #         'optimizer': optimizer.state_dict()
    #     }, args.checkpoint_dir, r.stage, 0)
    # checkpoint_file_path = os.path.join(args.checkpoint_dir, "checkpoint.%d.pth.tar.epoch.%d" % (r.stage, 0))
    ...
    #
    # if r.stage==0:
    #     checkpoint_file_path = os.path.join(args.checkpoint_dir, "checkpoint.%d.pth.tar.epoch.%d" % (1, 0))
    #     # checkpoint_file_path = "%s.%d.pth.tar" % (args.resume, r.stage)
    #     print(checkpoint_file_path)
    #     assert os.path.isfile(checkpoint_file_path)
    #     print("=> loading checkpoint '{}'".format(checkpoint_file_path))
    ...
    #           .format(checkpoint_file_path, checkpoint['epoch']))
    # if r.stage==1:
    #     checkpoint_file_path = os.path.join(args.checkpoint_dir, "checkpoint.%d.pth.tar.epoch.%d" % (0, 0))
    #     # checkpoint_file_path = "%s.%d.pth.tar" % (args.resume, r.stage)
    #     print(checkpoint_file_path)
    #     assert os.path.isfile(checkpoint_file_path)
    #     print("=> loading checkpoint '{}'".format(checkpoint_file_path))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # model1 = module.model(criterion, [1,1,46, 1], partition["recompute_ratio"])
        ...
        if args.resume:
            ...
            # assert os.path.isfile(checkpoint_file_path)
            # if r.rank == 0:
            #     r.Send_param("checkpoint.0.pth.tar.epoch.0",1)
            # if r.rank == 1:
            #     r.Rec_param("checkpoint.0.pth.tar.epoch.0_save",0)
...
def train(train_loader, r, optimizer, epoch, lr_scheduler):
    ...
    if args.worker_num_sum == 4:
        n = 17293
    elif args.worker_num_sum == 2:
        n = 8979
    ...
    def pipelining(steps, print_freq, time_for, time_bac, time_bac_begin, time_last_f, time_last_b, weight_stash=True):
        ...
        for i in range(steps - num_warmup_minibatches):
            # if i==i_for_initial-num_warmup_minibatches and i_for_initial!=0:
            # if i==109-num_warmup_minibatches and epoch==-1:
            #     print("begin")
            ...
            time1 = time.time()
            if i == steps - num_warmup_minibatches - 1:
                r.run_forward(stopped=True)
            else:
                r.run_forward()
            adjust_learning_rate(
                optimizer, epoch, args.epochs, r, args.lr_policy, i, steps)
            ...
            if is_last_stage():
                ...
                if s % print_freq == 0 and s > warmup_steps:
                    ...
                    batch_time.update(
                        (time.time() - batch_start_time) / print_freq)
                    batch_start_time = time.time()
                    epoch_time = (time.time() - epoch_start_time) / 3600.0
                    full_epoch_time = (
                        epoch_time / float(s - warmup_steps)) * float(n)
                    train_loss.append(loss)
                    if is_last_stage():
                        with open("./trainloss_3.19.txt", 'a') as train_los:
                            train_los.write(f"{losses.avg}\n")
                ...
                if s % print_freq == 0 and s > warmup_steps:
                    ...
                    print('Stage: [{0}] Epoch: [{1}][{2}/{3}]\tMemory: {memory:.3f} ({cached_memory:.3f})'.format(
                        args.stage, epoch, s, steps, memory=(
                            float(torch.cuda.memory_allocated()) / 10 ** 9),
            ...
            if not weight_stash:
                time2 = time.time()
                if i == steps - num_warmup_minibatches - 1 and num_warmup_minibatches == 0:
                    r.run_backward(stopped=True)
                else:
                    r.run_backward()
            ...
            if i % 10 == 0 and i >= 60:
                # if r.stage==3 and i==200:
                #     r.i_for_initial=torch.tensor([210])
                # if r.stage==3 and i>100:
                #     r.previous_status=r.status
                # r.Send_initial(i)
                # r.Rec_initial(i)
                # i_for_initial=r.i_for_initial[0]
                # print(i_for_initial)
                # r.status[r.stage]=time_for+time_bac
                # r.Send_Status(i)
                # r.Rec_Status(i)
                # print(r.status)
                # if r.stage==3:
                #     max_index=torch.argmax(r.status)
                #     if r.previous_status[max_index]<(2/3)*r.status[max_index] or (2/3)*r.previous_status[max_index]>r.status[max_index]:
                #         start_id=0
                ...
                time_list_last_f.append(time_last_f)
                time_list_last_b.append(time_last_b)
            ...
            if i % 100 == 0 and i > 0:
                ...
                if r.stage == 3:
                    ...
                    save_list_to_txt(time_list_batch, "data_batch")
                    save_list_to_txt(time_list_last_f, "data_last_f")
                    save_list_to_txt(time_list_last_b, "data_last_b")
    ...
    if args.sync_mode == BSP:
        ...
        print_freq = (args.print_freq // accumulation_steps) * \
            accumulation_steps
        ...
        pipelining(n, args.print_freq, time_for, time_bac,
                   time_bac_begin, time_last_f, time_last_b, weight_stash=True)
...
def save_checkpoint(state, checkpoint_dir, stage, epoch):
    ...
    checkpoint_file_path = os.path.join(
        checkpoint_dir, "checkpoint.%d.pth.tar.epoch.%d" % (stage, epoch))
...
def adjust_learning_rate(optimizer, epoch, total_epochs, r, lr_policy, step, epoch_length):
    ...
    if epoch <= 2:
        ...
        if lr_policy == "step":
            ...
            if epoch >= 2:
                decay_rate = 0.97
                lr = 0.01*(decay_rate**(epoch-2))
            else:
                lr = 5*stage_base_lr * 10**(-epoch+1)
