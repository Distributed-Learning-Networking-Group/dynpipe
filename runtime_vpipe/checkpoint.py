...
class CheckpointFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, run_function, fwd_ctxs, *args):
        preserve_rng_state = True
        check_backward_validity(args)
        ctx.recomputed = False
    ...
    def backward(ctx, *args):
        ...
        outputs = ctx.outputs
        detached_inputs = ctx.saved_tensors
...
def pre_backward(ctx):
    ...
    with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
        ...
        with torch.enable_grad():
            ctx.outputs = ctx.run_function(*detached_inputs)
        ctx.save_for_backward(detached_inputs)

def checkpoint(function, ctxs, *args, **kwargs):
    return CheckpointFunction.apply(function, ctxs, *args)
