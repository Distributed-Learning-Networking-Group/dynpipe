...
if __name__ == "__main__":
    ...
    with open(args.config_file, 'r') as stream:
        configurations = yaml.safe_load(stream=stream)
    ...
    for node_rank, (node_ip, workers) in \
        ...
        print("node_rank")
        print(node_rank)
        docker_cmd = 'docker run -it --gpus all %(mount_directories)s ' \
                        '--net=host ' \
        ...
        runtime_cmd = "export GLOO_SOCKET_IFNAME=docker0; " + " ".join(runtime_cmd_list) + "; rm launch.py"
        ...
        if not args.quiet:
            if node_ip != 'localhost' and node_ip != '127.0.0.1':
                processes.append(subprocess.Popen(launch_cmd, shell=True, stdout=subprocess.
                                                  PIPE))
            else:
